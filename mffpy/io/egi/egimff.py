...
def _read_mff_header(filepath):
    ...
    # Add the sensor info.
    sensor_layout_file = join(filepath, 'sensorLayout.xml')
    ...
    if 'PNS' in all_files:
        pns_fpath = join(filepath, all_files['PNS']['signal'])
        pns_blocks = _get_blocks(pns_fpath)

        pns_file = join(filepath, 'pnsSet.xml')
    ...
    info_filepath = join(filepath, 'info.xml')  # add with filepath
...
def _read_locs(filepath, chs, egi_info):
    ...
    fname = join(filepath, 'coordinates.xml')
    if not exists(fname):
        return chs
